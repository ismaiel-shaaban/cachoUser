...
import { FILE_URL, dataLangTranslation } from "src/Utils/Helpers";
import useAuthValue from "src/Modules/AuthModule/Hooks/useAuthValue";
import { strings } from "src/Utils/Localization";
type OFFERS_LIST_PROPS = {
  item: any;
};

function OffersListing(props: OFFERS_LIST_PROPS) {
  const { item } = props;
  ...
  return (
    <article
      className={
        language === "ar"
          ? ` ${styles.categoryCardItem}  ${styles.arabicCategoryCardItem} `
    ...
    >
        ...
        <Image
          src={
            item?.productId?.photos
              ? `${FILE_URL}${item?.productId?.photos[0]}`
              : images.cateItem
          }
          alt={`card item image`}
          layout="fill"
      ...
      <div className={styles.cardDetails}>
        <h3 className={styles.cardTitle}>
          {dataLangTranslation(item?.productId?.name, item?.productId?.arName)}
        </h3>
        <h4 className={styles.price}>SAR{item?.productId?.price}</h4>
        <div className={styles.offer}>
          ...
          {`${item?.offerId?.discount}% ${strings?.offer_active}`}
        </div>
      </div>
    </article>
  );
